...
export const INITIAL_CARDS: CardContent[] = [
  {
    ...
    title: "璀璨星光",
    text: "传说在北欧森林深处，有一棵古老的冷杉。每当平安夜降临，森林精灵便会收集星尘洒落在树梢。这棵树代表着永恒的生命与希望。当你看见树顶的光芒时，不管身在何处，温暖都会指引你回家的路。",
    imageIcon: "tree",
...
const PARTICLE_COUNT = 3200; // Increased count slightly for denser look
...
export const generateShapeData = (shape: ShapeType): { positions: Float32Array, colors: Float32Array } => {
  ...
  for (let i = 0; i < PARTICLE_COUNT; i++) {
    ...
    if (shape === ShapeType.TREE) {
      // --- TREE (Reference Size) ---
      // Tree is roughly 4.5 units tall
      const scale = 0.75;
      ...
      if (i < 50) {
        ...
        x += jitter(0.15); y += jitter(0.15); z += jitter(0.15);
        colorHelper.setHSL(0.3 + Math.random() * 0.05, 0.8, 0.2 + Math.random() * 0.4);
      }
    } 
    else if (shape === ShapeType.SLEIGH) {
      // --- SLEIGH ---
      // Further increased scale to match Tree volume visually
      const scale = 1.55; 
      
      // Vertical adjustment to lift Sleigh up
      ...
      if (i < 800) {
        ...
        const t = (i / 800); 
        const zPos = (t - 0.5) * 4.0; 
        let yPos = -1.5;
        if (zPos > 1.0) yPos += Math.pow(zPos - 1.0, 2) * 0.5;

        x = side * 0.8 + jitter(0.08); 
        y = yPos + jitter(0.08);
        z = zPos + jitter(0.08);
        colorHelper.set('#C0C0C0'); 
      } else if (i < 2200) {
        // Body
        const t = Math.random(); 
        const u = Math.random(); 
        const zPos = (t - 0.5) * 3.0; 
        const angle = (u - 0.5) * Math.PI; 
        
        let radius = 0.9;
        let yOffset = -0.8;
        if (zPos < -1.0) yOffset += ( -1.0 - zPos) * 0.5; 
        
        x = Math.sin(angle) * radius + jitter(0.1);
        y = Math.cos(angle) * radius * 0.6 + yOffset + jitter(0.1);
        z = zPos + jitter(0.1);
        colorHelper.set('#8B0000'); 
      } else {
        // Inside
        const t = Math.random();
      ...
      // Apply scale AND vertical lift here
      x = xN * scale; 
      y = (yN * scale) + verticalLift; // Shift UP
      z = zN * scale;

    } 
    else if (shape === ShapeType.GIFT) {
      // --- GIFT ---
      // Box size 3.6 makes it very substantial
      const boxSize = 3.6; 
      ...
      if (i < 2200) {
        ...
        const depth = (Math.random()) * 0.15; // Slightly thicker walls
        ...
        x += jitter(0.08); y += jitter(0.08); z += jitter(0.08);
        colorHelper.set('#b71c1c'); 
        
      } else if (i < 2600) {
        // Ribbons
        const axis = i % 2; 
        const half = boxSize / 2 + 0.05;
        const bandW = 0.5 * (boxSize / 2); 
        
        if (axis === 0) { 
           const r = Math.random();
           if(r<0.33) { x=(Math.random()-0.5)*bandW; y=half; z=(Math.random()-0.5)*boxSize; } 
           else if(r<0.66) { x=(Math.random()-0.5)*bandW; y=(Math.random()-0.5)*boxSize; z=half; } 
           else { x=(Math.random()-0.5)*bandW; y=(Math.random()-0.5)*boxSize; z=-half; } 
        } else { 
            const r = Math.random();
            if(r<0.33) { z=(Math.random()-0.5)*bandW; y=half; x=(Math.random()-0.5)*boxSize; }
            else if (r < 0.66) { z=(Math.random()-0.5)*bandW; x=half; y=(Math.random()-0.5)*boxSize; }
            else { z=(Math.random()-0.5)*bandW; x=-half; y=(Math.random()-0.5)*boxSize; }
        }
        
        x += jitter(0.04); y += jitter(0.04); z += jitter(0.04);
        colorHelper.set('#ffd700'); 

      } else {
        // Bow
        const t = Math.random() * Math.PI * 2;
        const scaleBow = 0.35 * boxSize; 
        const angle = t;
        const r = Math.sin(2 * angle) * scaleBow;
        
        x = r * Math.cos(angle);
        z = r * Math.sin(angle);
        y = (boxSize / 2) + 0.2 + Math.abs(r) * 0.3; 
